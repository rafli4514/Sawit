...
export default function EditorAturanPage() {
  const params = useParams();
  const ruleId = params?.ruleId as string | undefined;
  const router = useRouter();
  const isNew = !ruleId || ruleId === "baru";
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      if (!isNew) {
        ...
        const existing = allRules.find((r: any) => r.id === Number(ruleId));
        
        if (existing) {
          setCode(existing.penyakit.kode);
          setTitle(`Aturan untuk ${existing.penyakit.nama}`);
          setAntecedents([{ symptomId: existing.gejalaId, minConfidence: 0.3 }]);
          setConsequents([{ diseaseId: existing.penyakitId, cf: existing.cfPakar }]);
        }
      }
    } catch (error) {
  ...
  const handleSimulate = () => {
    ...
    const mockRule: Rule = {
      id: Number(ruleId) || 999, code, title, antecedents, consequents, notes,
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-lg font-black text-slate-900 leading-tight">
                {isNew ? "Aturan Baru" : "Edit Aturan"} <span className="text-[#006837] ml-1">{code || "Pakar"}</span>
              </h1>
              ...
              <CardContent className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <Input 
                    ...
                    className="h-10 rounded-lg border-slate-200 bg-slate-50/30 focus:bg-white font-bold text-xs"
                  />
                </div>
                <div className="space-y-2">
                ...
                <div className="sm:col-span-2 space-y-2">
                  ...
                  <textarea
                    ...
                    className="w-full border-slate-200 bg-slate-50/30 rounded-lg px-4 py-3 text-xs min-h-[80px] focus:outline-none focus:ring-1 focus:ring-[#8DC63F] font-medium text-slate-600"
                    placeholder="Detail pendukung atau referensi pakar..."
                  />
                </div>
              </CardContent>
            </Card>

            {/* 2. Gejala (Anteseden) Card */}
            <Card className="rounded-xl border-slate-200 shadow-sm bg-white">
                    ...
                    <div key={i} className="flex items-start gap-4 p-4 rounded-xl border border-slate-100 bg-white shadow-sm hover:border-[#8DC63F]/30 transition-all">
                      <div className="flex-1 space-y-4">
                        ...
                        <div className="flex items-center gap-4 bg-slate-50/50 p-2.5 rounded-lg border border-slate-100">
                          ...
                          <Slider
                            value={[Math.round(ant.minConfidence * 100)]}
                            min={0} max={100} step={5}
                            onValueChange={([v]) => updateAntecedent(i, "minConfidence", v / 100)}
                            className="flex-1"
                          />
                        </div>
                      </div>
                    ...
                    <div key={i} className="flex items-start gap-4 p-4 rounded-xl border border-amber-100 bg-amber-50/5 hover:bg-white transition-all shadow-sm">
                      <div className="flex-1 space-y-4">
                        ...
                        <div className="flex items-center gap-4 bg-white p-2.5 rounded-lg border border-slate-100">
                          ...
                          <Slider
                            value={[Math.round(cons.cf * 100)]}
                            min={-100} max={100} step={5}
                            onValueChange={([v]) => updateConsequent(i, "cf", v / 100)}
                            className="flex-1"
                          />
                        </div>
                      </div>
              ...
              <CardContent className="p-0 flex-1 flex flex-col min-h-0">
                <div className="p-6 space-y-5 flex-1 overflow-y-auto no-scrollbar">
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <Button onClick={addSandboxInput} variant="ghost" className="h-6 px-2 rounded text-[8px] font-black uppercase hover:bg-slate-100">
                          ...
                          <div key={i} className="p-3 rounded-lg bg-slate-50 border border-slate-100 space-y-2 group transition-all hover:bg-white hover:shadow-sm">
                            <div className="flex items-center justify-between gap-2">
                              <Select value={String(inp.symptomId)} onValueChange={(v) => updateSandboxInput(i, "symptomId", Number(v))}>
                                <SelectTrigger className="h-7 rounded border-transparent bg-white/50 font-bold text-[9px] flex-1 shadow-none">
                                  <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                                  {apiSymptoms.map((s) => (
                  ...
                  {simResult && (
                    <div className={`rounded-xl border p-5 space-y-4 animate-in fade-in zoom-in-95 duration-300 ${
                      simResult.fires ? "bg-green-50/50 border-green-200" : "bg-red-50/50 border-red-200"
                    }`}>
                      <div className="flex items-center gap-3">
                        <div className={`h-8 w-8 rounded-full flex items-center justify-center ${simResult.fires ? "bg-green-500 text-white shadow-lg shadow-green-500/20" : "bg-red-500 text-white shadow-lg shadow-red-500/20"}`}>
                          <Play className="h-4 w-4 fill-current ml-0.5" />
                        </div>
                        <span className={`font-black text-[11px] uppercase tracking-widest ${simResult.fires ? "text-green-800" : "text-red-800"}`}>
                          {simResult.fires ? "Aturan Aktif" : "Terhenti"}
                        </span>
                      </div>
                      ...
                      {simResult.fires && (
                        <div className="flex items-end justify-between border-t border-green-200/50 pt-3">
                          <div>
                            <p className="text-[8px] font-black text-slate-400 uppercase tracking-tighter mb-0.5">CF Kontribusi</p>
                            <p className="text-2xl font-black text-green-700 leading-none">{(simResult.cfContribution * 100).toFixed(1)}%</p>
                          </div>
                          ...
                          return (
                            <div key={match.symptomId} className="flex items-center justify-between text-[8px] font-bold">
                              ...
                              <div className="flex items-center gap-1 font-mono">
                                ...
                                <span className="text-slate-400">{Math.round(match.minConfidence * 100)}%</span>
                              </div>
                            </div>
                          );
                        })}
                ...
                <div className="p-6 border-t border-slate-100 bg-slate-50/50 shrink-0">
                  <Button 
                    ...
                    className="w-full h-11 bg-[#006837] hover:bg-[#004d2c] text-white rounded-xl font-black uppercase tracking-[0.2em] text-[10px] shadow-lg shadow-green-900/10 transition-all active:scale-[0.98]"
                  >
                    Jalankan Tes Logika
                  </Button>
                </div>
